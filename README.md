# Frontend Mastery

Personal learning journey from JavaScript fundamentals to production-ready frontend engineering.

## Current Focus
**Phase 1  JavaScript Foundations**

## Structure
- `phase-01-js-foundations/` → Language fundamentals
- `phase-02-dom-browser/` → DOM & Browser APIs
- `phase-03-async-apis/` → Asynchronous JS & Public APIs
- `phase-04-tooling/` → Modules, Vite, tooling
- `phase-05-react/` → React foundations
- `phase-06-fullstack/` → Django + DRF + React
- `phase-07-react-native/` → Mobile
- `phase-08-production/` → Hardening & advanced topics

## Rules
1. Every concept is learned by building.
2. No moving to the next phase until the current project is:
   - Built
   - Documented
   - Explained in my own words
   - Rebuilt from memory
3. Prefer deep understanding over speed.

## Progress
- [x] Phase 1: JavaScript Foundations
- [ ] Phase 2: DOM & Browser
- [ ] Phase 3: Async + APIs
- [ ] Phase 4: Tooling
- [ ] Phase 5: React
- [ ] Phase 6: Full-Stack
- [ ] Phase 7: React Native
- [ ] Phase 8: Production